...
### 7. Chrome起動＆ログイン（CDP方式）
WEB操作は **既に起動済みのChrome** に Playwright がつなぐ方式です（CDP接続）。
セッションは Chrome 任せなので、保存パスワードでの自動再ログインが効きます。

```cmd
start_chrome.bat をダブルクリック
```
→ 専用プロフィール（`%LOCALAPPDATA%\bukken_cleaning_chrome`）で Chrome 起動
→ スカイヤーズ・ピタクラのログインタブが自動で開く
→ 各サイトにログイン → Chrome が「パスワードを保存しますか？」と聞いてくる → **保存**

以降は Chrome を開きっぱなしにすればよく、セッション切れ時も Chrome の保存
パスワードで autofill されます。Chrome を閉じてしまっていたら再度 `start_chrome.bat`。
